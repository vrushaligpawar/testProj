...
public class TC12_PS_ProposalGenerate {
	...
	PSAddIntegratedMobileTraiff pimt=new PSAddIntegratedMobileTraiff();
	SubmitForApproval sfa=new SubmitForApproval();
	ApprovalStatus as=new ApprovalStatus();
	CommonErrorMsg cem= new CommonErrorMsg();
	CreateProposal cp=new CreateProposal();
	
		
	@BeforeMethod
	public void beforeMethod() throws IOException, InterruptedException
	{		
		reports=new ExtentReports("D:\\Workspace\\CPQ\\src\\Report\\TC12_PS_ProposalGenerate.html",true);
		logger=reports.startTest("TC12_PS_ProposalGenerate");
		CommonActions.LaunchDriver.launchbrowser();
		logger.log(LogStatus.PASS, "browser launch successfully");
	}
	
	@Test
	public void proposalGenerate() throws IOException, InterruptedException, AWTException
	{
		...
		pimt.setUniqueID("UKTraiff1");pimt.addIntegratedMobileTraiffPS();//add integrated mobile tariff
		...
		noc.editQuotelink();
		cp.proposalVerify();
		if(cp.verifyproposalOpen()){
			logger.log(LogStatus.FAIL, "Proposal not get open");
		}else
		{
			logger.log(LogStatus.PASS, "Proposal open successfully");
		}
				}
