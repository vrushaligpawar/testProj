...
public class TC18_SME_FixedProduct_VSDM_SelfApproval {
	...
	{		
		reports=new ExtentReports("D:\\Workspace\\CPQ\\src\\Report\\TC18_SME_FixedProduct_VSDM_SelfApproval.html",true);
		logger=reports.startTest("TC18_SME_FixedProduct_VSDM_SelfApproval");
		CommonActions.LaunchDriver.launchbrowser();
		logger.log(LogStatus.PASS, "browser launch successfully");
	}
	
	@Test
	public void SMESelfApprovalFixed() throws IOException, InterruptedException, AWTException
	{
		la.setUniqueID("UK_AM");
		la.FillLoginInfo();	//Login
		logger.log(LogStatus.PASS, "Login successfully with UK QaMaster");
		nac.setUniqueID("UK_SME"); 
		nac.clickAccountTab();
		nac.AccountCreationInfo();//New account creation	
		System.out.println(nac.getAccountName());
		logger.log(LogStatus.PASS, "SME Account created successfully");
